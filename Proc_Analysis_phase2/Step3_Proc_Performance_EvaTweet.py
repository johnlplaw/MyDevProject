...
m_mBERT_syn_slbl = Selected_Model("Final-mBERT_SLBL_Multilingual_2800_1e-05_model.pth","mBERT-syn-lsbl", lib.COL_SYN_SLBL_MBERT)
...
for dsType in dsTypeList:
    for labelType in labelingTypeList:
        ...
        labelList = lib.get_Label_List_tweets(tableName, label)
        for model in mbert_model_list:
            print("DatasetType: " + dsType + ", LabelType:" + labelType)
            print("Working on: " + model.modelName + " ... start")

            predicted_label = lib.get_Label_List_tweets(tableName, model.colName)
            print(len(labelList))
