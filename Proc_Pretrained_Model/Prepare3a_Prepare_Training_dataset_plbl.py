...
def create_training_eng (sampling_size):
    ...
    source_file = var.FILE_MULTICOMB_DF_PLBL + str(sampling_size) + ".obj"
...
#for size in [var.SAMPLING_3200, var.SAMPLING_3600]:
    ...
    fileObj = open(var.FILE_TRAINING_ENG_NOS_PLBL + str(size) + ".obj", 'wb')
    pickle.dump(eng_df, fileObj)
    fileObj.close()
    fileObj = open(var.FILE_TRAINING_MUL_NOS_PLBL + str(size) + ".obj", 'wb')
